...
\end{equation}
\defaultcite{SR75} proves the existence of a flow map $U(t,t_0)$ which is a solution to \ref{num_B}. The unitary representation
\begin{align} \label{eq_rho}
  \rho : SO(d) &\longrightarrow U(L^2(\mathbb{R}^d)) \\
  R &\longmapsto (\rho (R)\Psi)(x) = \Psi(R^{-1} x)
\end{align}
maps the solution $U(t, t_0)$ of \ref{num_B} to a solution of \ref{num_M}. The proof of this statement can be found in \defaultcite{GR18}. The exact flow map $U(t, t_0)$ can be approximated by the Magnus expansion proposed by Blanes and Moan \defaultcite{BM06}.
...
\end{equation}
in the following way: Denote the solutions to \ref{num_kin} and \ref{num_M} by $\Phi_{-\Delta}$ and $\Phi_{H_B}$ respectively. The flow map $\Phi_{-\Delta + H_B}$ which is a solution to \ref{num_K_M} then follows as a simple multiplication
\begin{equation*}
...
\end{equation*}
to an initial time $t_0$. \defaultcite{GR18} derives an explicit expression of the solution to \ref{num_main} as follows:
...
\end{multline}
where $h$ is the splitting time step, $\rho$ the representation defined in \cref{eq_rho} and $U(t, t_0)$ the exact flow map to \ref{num_B}. \newline
...
From \cref{fig:convthreefold} and \cref{fig:convtorsional} we can conclude that the method is well suited for values of $\epsilon > 0.125$, but is subject to increasing errors for values of $\epsilon$ smaller than that.\newline
