...
class FourierMagneticPropagator(Propagator, SplittingParameters):
    ...
    def post_propagate(self, tspan):
        ...
        :param tspan: ``ndarray`` consisting of end time at position 0, other positions are irrelevant.
        """
        ...
        #calculate timesteps
        # each j is an individual time steps
        for j in range(nsteps):
            # each i is an intermediate time step in the splitting
            for i in range(len(self._a)):

                ### Calculate potential flow map \Phi_V = \Phi_B * \Phi_{\phi}###

                # Integral -\int_{tspan[0]}^{tspan[1]}B^2(s)ds and its associated propagation
                # does not need to be rotated, as B^2(s) is independent of rotations
                # (see [Gradinaru and Rietmann, 2020]. Remark 3.1)
                ...
                #define the magnetic field propagator \Phi_B = \int{ -i/eps^2 * \int{ B^2}}
                prop = exp(-1.0j / self._eps**2 * VB)

                #apply the propagator
                values = self._psi.get_values()
                values = [prop * component for component in values]

                #define the propagator \Phi_{\phi} (for electric and spatial potential)
                # these potential are not independent of rotations and need to be rotated
                self._potential.calculate_exponential(-1.0j *  self._a[i]*self._dt /self._eps**2)

                # rotate and evaluate potentials
                self._grid.rotate(R.T)
                VE = self._potential.evaluate_exponential_at(self._grid)
                self._VE = tuple([ve.reshape(self._grid.get_number_nodes()) for ve in VE])
                ...
                # define time step for Magnus Integrator
                t_interval[0] = t_b
                t_interval[1] = t_b + self._b[i]*self._dt

                # calculate Magnus Integrator
